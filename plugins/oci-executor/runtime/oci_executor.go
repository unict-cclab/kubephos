...
func installCommand(marker string, target machine) string {
	...
	parts := []string{
		...
		"attempt=0; until docker --host tcp://" + address + ":2376 --tlsverify --tlscacert \"$executor_home/.config/docker/tls/ca.pem\" --tlscert \"$executor_home/.config/docker/tls/client-cert.pem\" --tlskey \"$executor_home/.config/docker/tls/client-key.pem\" info --format '{{json .SecurityOptions}}' | grep -q rootless; do attempt=$((attempt + 1)); test \"$attempt\" -lt 61; sleep 2; done",
