...
func TestTopologyPlanDeclaresAtomicMachineSet(t *testing.T) {
	...
	plan, err := (TopologyPlugin{}).Plan(context.Background(), spec)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTopologyCreatesVerifiesAndRemovesOnlyPlannedMachines(t *testing.T) {
	...
	defer server.Close()
	invocation, spec := topologyInvocation(server.URL, true)
	...
	for _, step := range plan.Steps {
		health, err := (TopologyPlugin{}).Precheck(context.Background(), step, invocation.Secrets, invocation.Connections, log)
		if err != nil || health.Status != domain.HealthHealthy {
			t.Fatalf("precheck failed %#v %v", health, err)
		}
		result, err := (TopologyPlugin{}).Execute(context.Background(), step, invocation.Secrets, invocation.Connections, log)
		if err != nil {
			t.Fatal(err)
		}
		...
		health, err = (TopologyPlugin{}).Verify(context.Background(), step, result, invocation.Secrets, invocation.Connections, log)
		if err != nil || health.Status != domain.HealthHealthy {
...
func TestTopologyRollsBackCreatedMachinesOnPartialFailure(t *testing.T) {
	...
	defer server.Close()
	invocation, spec := topologyInvocation(server.URL, false)
	plan, err := (TopologyPlugin{}).Plan(context.Background(), spec)
	if err != nil {
		t.Fatal(err)
	}
	_, err = (TopologyPlugin{}).Execute(context.Background(), plan.Steps[0], invocation.Secrets, invocation.Connections, func(string, string) error { return nil })
	if err == nil {
...
func newTopologyServer(t *testing.T, failVMID int) (*httptest.Server, *topologyServerState) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case request.Method == http.MethodGet && strings.HasSuffix(request.URL.Path, "/config"):
			...
			value, _ := json.Marshal(map[string]any{"data": map[string]string{"name": machine.name, "tags": machine.tags, "description": machine.description, "ciuser": machine.ciUser, "sshkeys": machine.sshKeys, "ipconfig0": machine.ipConfig0}})
			_, _ = response.Write(value)
