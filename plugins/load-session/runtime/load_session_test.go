...
func testArtifacts(t *testing.T) map[string]domain.ResolvedArtifact {
	...
	manifest := "apiVersion: apps/v1\nkind: Deployment\nmetadata:\n  name: loadgenerator\nspec:\n  replicas: 1\n  selector:\n    matchLabels:\n      app: loadgenerator\n  template:\n    metadata:\n      labels:\n        app: loadgenerator\n    spec:\n      containers:\n      - name: loadgenerator\n        image: example.invalid/load@sha256:" + strings.Repeat("b", 64) + "\n"
	manifestHash := sha256.Sum256([]byte(manifest))
	profiles := []loadProfile{{ID: "default-load", TargetEndpoint: "frontend-http", Replicas: 1, Workload: workloadTarget{ID: "default-load", APIVersion: "apps/v1", Kind: "Deployment", Name: "loadgenerator", Selector: map[string]string{"app": "loadgenerator"}}, Manifest: manifest, ManifestDigest: "sha256:" + hex.EncodeToString(manifestHash[:])}}
	digest, err := digestJSON(profiles)
	if err != nil {
		t.Fatal(err)
	}
	profileSet, _ := json.Marshal(map[string]any{"apiVersion": artifactAPI, "kind": "LoadProfileSet", "metadata": map[string]string{"applicationRef": "app:dev.example.app@1.0.0", "digest": digest}, "spec": map[string]any{"profiles": profiles}})
	return map[string]domain.ResolvedArtifact{"cluster-connection": {Value: cluster}, "application-deployment": {Value: deployment}, "load-profile-set": {Value: profileSet}}
}
...
func (runner *fakeRunner) Run(_ context.Context, _ string, stdin []byte, args ...string) (string, error) {
	...
	if strings.HasPrefix(command, "get deployment/loadgenerator ") {
		if !runner.exists {
			return "", nil
		}
	...
	if strings.HasPrefix(command, "scale deployment/loadgenerator ") {
	...
	if strings.HasPrefix(command, "get pods ") {
		...
		return "pod/loadgenerator-one", nil
	}
	if strings.HasPrefix(command, "delete deployment/loadgenerator ") {
		runner.exists = false
		return "deleted", nil
	}
